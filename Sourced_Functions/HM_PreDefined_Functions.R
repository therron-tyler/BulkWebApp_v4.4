...
groupfile_reformat <- function(group_file) {
  ...
  # print("the group file HM")
  # print(group_df_t)
  
  if (all(c("V1", "V2") %in% colnames(group_df_t))) {
    organized_hm_groups <- data.frame(Sample = row.names(group_df_t), Group = group_df_t$V1, Color = group_df_t$V2)
  } else {
...
# II
generate_space_text <- function(num_columns, base_space = 13, scaling_factor = 1.5) {
  # Dynamically adjust spacing based on the number of columns
  space_multiplier <- floor(num_columns / (10 * scaling_factor))  
  total_space <- base_space + space_multiplier
...
# II
remove_space_text <- function(num_columns, base_space = 10, scaling_factor = 1.2) {
  # Dynamically adjust spacing and prevent negative values
  space_multiplier <- floor(num_columns / (6 * scaling_factor))
  total_space <- max(base_space - space_multiplier, 2)  # Ensure minimum spacing
